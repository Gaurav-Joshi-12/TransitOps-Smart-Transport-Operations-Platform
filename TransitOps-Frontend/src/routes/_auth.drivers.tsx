...
import { createDriver, updateDriver, updateDriverStatus } from '@/services/api'
...
function DriversPage() {
  ...
  const canEdit = isFleetManager || isSafetyOfficer
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      if (editing) {
        if (isSafetyOfficer) {
          await updateDriverStatus(editing.id, form.status, form.safetyScore)
          toast.success('Driver status and safety score updated')
        } else {
    ...
    <div className="space-y-4">
      <DataTable
        ...
        toolbar={
          <RoleGuard allow={['FLEET_MANAGER']}>
            <Button size="sm" onClick={openNew}><Plus className="mr-1.5 h-4 w-4" /> Add driver</Button>
          </RoleGuard>
        }
      />
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>
              {editing ? (isSafetyOfficer ? 'Update Driver Status' : 'Edit driver') : 'Add driver'}
            </DialogTitle>
          </DialogHeader>
          <form onSubmit={onSubmit} className="grid grid-cols-2 gap-4">
            <div className="col-span-2 space-y-2">
              <Label>Full name</Label>
              <Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required disabled={!isFleetManager} />
            </div>
            <div className="space-y-2">
              <Label>License No.</Label>
              <Input value={form.licenseNo} onChange={(e) => setForm({ ...form, licenseNo: e.target.value })} required disabled={!isFleetManager} />
            </div>
            <div className="space-y-2">
              <Label>Category</Label>
              <Select value={form.licenseCategory} onValueChange={(v) => setForm({ ...form, licenseCategory: v })} disabled={!isFleetManager}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  {['LMV', 'HMV', 'MCWG', 'C', 'D'].map((c) => <SelectItem key={c} value={c}>{c}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-2">
              <Label>License expiry</Label>
              <Input type="date" value={form.licenseExpiry} onChange={(e) => setForm({ ...form, licenseExpiry: e.target.value })} required disabled={!isFleetManager} />
            </div>
            <div className="space-y-2">
              <Label>Contact</Label>
              <Input value={form.contact} onChange={(e) => setForm({ ...form, contact: e.target.value })} required disabled={!isFleetManager} />
            </div>
            <div className="space-y-2">
