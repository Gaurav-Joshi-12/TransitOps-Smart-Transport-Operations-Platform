...
function MaintenancePage() {
  ...
  async function onCreate(e: React.FormEvent) {
    e.preventDefault();
    try {
      await openMaintenance(form.vehicleId, form.description);
      toast.success("Maintenance log opened — vehicle set to In Shop");
      setOpen(false); setForm({ vehicleId: "", description: "" });
  ...
  return (
    <div className="space-y-4">
      <DataTable
        ...
        toolbar={<Button size="sm" onClick={() => setOpen(true)}><Plus className="mr-1.5 h-4 w-4" /> Open log</Button>}
      />
